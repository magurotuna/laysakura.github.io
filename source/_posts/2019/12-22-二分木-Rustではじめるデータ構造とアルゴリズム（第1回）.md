...
1. <a href="/2019/12/22/rust-DataStructures-Algorithm-BinaryTree">二分木</a> **（この記事）**
1. 二分探索木 **（未執筆）**
1. 平衡二分探索木 **（未執筆）**
1. ヒープ **（未執筆）**
1. 有向グラフ（ポインタ表現） **（未執筆）**
...
- Box<BinaryTree<T>> **（重要）**
    - _left: <BinaryTree<T>>_ のように書きたくなるが、これでは再帰的な型定義となってしまい、 BinaryTree 型のサイズが確定できない（「BinaryTree のサイズは... val で size_of<T\>() と、 left で BinaryTree のサイズを足して... ってそれを今計算してるのに😫」）。これを回避するために、"BinaryTree 型へのポインタ型" としてサイズが確定できるよう Box に包む。
...
解き方としては、愚直にルートからリーフまで足し算しながら足していって、リーフにたどり着いた時点で目指す総和になっていれば true を返せばよいです。
...
"上から下に" ではなく、 "下から上に" 考えてみると、「あるノードがリーフのとき、自分が0ならば、自分自身をNilに置き換える」という解放がとれます。これだと0との比較の回数は、多くても全ノードの数だけになります。
...
しかし二分木は、第3回に登場する二分探索木や、第4回のヒープのための基礎的なデータ構造になります。その助走となる次回第2回では、二分木の深さを各所大体一緒に保つ平衡二分木を紹介していきます。

<hr>

この記事は [FOLIO Advent Calendar 2019](https://qiita.com/advent-calendar/2019/folio-sec) の8日目の記事です。
12/23に8日目、開始は12/16日でしょうか？（原稿落とした同僚の代打投稿です）
